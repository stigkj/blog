...
["Derek Wyatt":http://derekwyatt.org/]'s @.vimrc@ file.)
...
+*Update*: There have been some passionate responses about this in comments, so
a warning may be appropriate here.  If you care about recovering after a Vim or
terminal emulator crash, or you often load huge files into memory, do *not*
disable the swapfile.  I personally save/commit so
...
p. <ins>*Update*: Another great trick I read in a "reddit
...
+*Update*: I used to have it mapped to @:let @/=""<CR>@, but some users kindly
pointed out that it is better to use @:nohlsearch@, because it keeps the search
history intact.+
...
that has a lot of nice features, but of course, lacks Vim-style cursor
navigation :)

The best feature of TextMate, I have always found the intelligent snippets: you
...
There are already lots of specific language plugins for snipMate available, but
writing your own is simple in case you ever miss functionality.  Just create a
file called @~/.vim/snippets/foo.snippet@, where @foo@ is the filetype you want
to load the snippets for.
